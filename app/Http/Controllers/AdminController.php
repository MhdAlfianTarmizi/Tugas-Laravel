...
class AdminController extends Controller
{
    ...
    public function postTambahAdmin(Request $request)
    {
        ...
        if ($saved) {
            return redirect('/admin/home')->with('success', 'Akun baru berhasil ditambah!');
        } else {
    ...
    public function editAdmin($id)
    {
        ...
        if (!$data) {
            return redirect()->route('admin.home')->with('failed', 'User not found.');
        }
    ...
    public function postEditAdmin(Request $request, $id)
    {
        ...
        if ($user) {
            return redirect('/admin/home')->with('success', 'Data admin berhasil di update!');
        } else {
    ...
    {
        // Membangun grafik peminjaman
        $chart = $chartPeminjaman->build();

        // Mengambil data pencarian dari permintaan
        $search = $request->input('search');

        // Mengambil data peminjaman berdasarkan kriteria pencarian
        $data = Peminjaman::where(function ($query) use ($search) {
            $query->where('id_user', 'LIKE', '%' . $search . '%');
        })->paginate(5);

        // Mengirim data ke tampilan admin.peminjaman.blade.php
    ...
    {
        return view('admin.tambahPeminjaman', compact('data', 'chart'));
    }
    ...
    {
        $detailPeninianan = Peminjaman::select('Peminjaman.*', 'buku.*', 'users.*')
